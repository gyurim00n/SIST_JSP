 ...
 <h3>Board 목록</h3>
  <table>
  	<caption style="text-align:right;" >
  		<a href="${pageContext.request.contextPath}/mvc/board/write.htm">글쓰기</a>
  	</caption>
  	<thead>
	  	<tr>
	  		<th width="10%">번호</th>
  		...
  		<c:otherwise>
  			<c:forEach items="${list}" var="dto">
  				<tr>
	  				<td>${dto.seq} </td>
	  				<td>
	  				<a href="${pageContext.request.contextPath}/mvc/board/view.htm?seq=${dto.seq}">${dto.title} </a>
	  				</td>
	  				<td>${dto.writer} </td>
	  				<td>${dto.writedate} </td>
	  				<td>${dto.readed} </td>
 ...
 */
	...
	return `${pageContext.request.contextPath}/mvc/board/list.htm`
	 + `?currentPage=\${oldHref}`
	 + `&numberPerPage=${param.numberPerPage}`
	 + `&searchCondition=${param.searchCondition}`
