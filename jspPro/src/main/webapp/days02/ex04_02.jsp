    ...
    <tfoot>
      <tr>
        <td colspan="8">
          ...
          <br />
          <button id="send">체크한 empno 전송</button>
          <!-- ex04_ok.jsp li empno -->
        </td>
      </tr>
    </tfoot>
  </table>
...
<script> //모두 선택 체크박스 처리 X (저번에 이미 했다..)
$("#ckbAll").on("click", function(){
	const checkAll = $("#ckbAll");
	const checklist = $("checkbox[id^=ckb]"); //ck로 시작하는 id
...
$("tfoot button").on("click", function(){
	const ckbs = $("tbody :checkbox:checked"); //띄어쓰기 하기...
	if(ckbs.length ==0 ){
		alert("사원 체크하시오");
		return;
	}
